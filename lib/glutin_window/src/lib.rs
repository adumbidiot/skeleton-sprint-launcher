...
fn context_builder_from_settings(
    ...
    let api = settings.get_maybe_graphics_api().unwrap_or_else(|| Api::opengl(3, 2));
    if api.api != "OpenGL" {
...
impl GlutinWindow {
    ...
    pub fn new(settings: &WindowSettings) -> Result<Self, Box<dyn Error>> {
        ...
        let ctx = match ctx {
                ...
                Err(_) => {
                    ...
                    let window_builder = window_builder_from_settings(&settings);
                    let context_builder = context_builder_from_settings(&settings)?;
                    context_builder.build_windowed(window_builder, &events_loop)?
                }
            };
    ...
    /// Creates a game window from a preexisting events_loop and window
    pub fn from_glutin(events_loop: glutin::EventsLoop, window_builder: glutin::WindowBuilder, settings: &WindowSettings) -> Result<Self, Box<dyn Error>> {
        let title = settings.get_title();
        let exit_on_esc = settings.get_exit_on_esc();
    ...
    fn fake_capture(&mut self) {
        if let Some(pos) = self.last_cursor_pos {
            ...
            let dx = cx - pos[0];
            let dy = cy - pos[1];
            if (dx != 0.0 || dy != 0.0) && self.ctx.window().set_cursor_position((cx, cy).into()).is_ok(){
...
impl Window for GlutinWindow {
    fn size(&self) -> Size {
        let size = self.ctx.window().get_inner_size().unwrap_or_else(|| (0.0, 0.0).into());
